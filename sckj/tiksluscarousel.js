   ...
   var TikslusCaraousel = function(element, options)
   {
	...
	//	var animating=false; //used in zoom effect
		...
		var defaults = {
                ...
                loader:'ajax-loader.gif'/*tpa=http://www.shjzlw.com/tpl/tianjilaowu/js/ajax-loader.gif*/,
...
carousel.find(".preload").css({position:'absolute',top:"48%",left:"35%",backgroudColor:'#fff',textAlign:"center",color:'#000',padding:'http://www.shjzlw.com/tpl/tianjilaowu/js/0.5em'}).show();
			   ...
			   var li = carousel.find("ul.carousel").find("li");
                li.each(function() {
                    if ($(this).find("img").attr("caption")) {
                        ...
                        $(this).find(".caption").css({left:0 + $(this).width() +  $(this).find(".caption").width() }).html(caption_html).hide();
				...
				var l=li.position().left;
                    li.find(".caption").css({left:0,fontSize:carousel.width()*options.captionFontRatio + "%"});
                    ...
                    li.find(".caption").css({fontSize:carousel.width()*options.captionFontRatio + "%"}).show().animate({left: 0});
        ...
        {//fullscreen is inactive
		 if(carousel.hasClass("fullscreen")){
           ...
           // code if fullscreen is active do nothing
			...
			carousel.find("http://www.shjzlw.com/tpl/tianjilaowu/js/.slider_nav .nav").css({width:  carousel.width()*options.dotRatio + "px", height: carousel.width()*options.dotRatio + "px"});
